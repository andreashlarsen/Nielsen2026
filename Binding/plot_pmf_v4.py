...
if IDP:
    pep = ['idp_b2','idp_d1','idp_kor2','idp_d22','idp_atr','idp_hec23']
    rep = [3,3,4,3,3,3]
    label = ['B2AR-d','D1R-d','KOR-d','D2R-d','AT1R-d','Hecate-d']
else:
#    pep = ['hecate','b2','atr','kor2','d22','d1','div4a','ashuffle','gshuffle']
    pep = ['b2','d1','kor2','d22','atr','hecate','div4a','ashuffle','gshuffle']
    rep = [3,3,3,3,3,3,0,0,0]
#    label = ['Hecate','B2AR','AT1R','KOR','D2R','D1R','DIV4a','Ashuffle','Gshuffle']
    label = ['B2AR','D1R','KOR','D2R','AT1R','Hecate','DIV4a','Ashuffle','Gshuffle']
#color = ['green','blue','cyan','orange','magenta','red','cyan','orange','grey','grey','grey','grey','grey']
color = ['blue','red','orange','magenta','cyan','green','grey','grey','grey']
...
for i in range(len(pep)):
    ...
    for r in range(rep[i]):
        ...
        if PLOT_ALL:
            ...
            if LABEL_FIRST_ONLY:
                if r == 1:
            	    plt.plot(d,E,color=color[i],label=r'%s' % label[i])
                else:
                    plt.plot(d,E,color=color[i])
            else:
